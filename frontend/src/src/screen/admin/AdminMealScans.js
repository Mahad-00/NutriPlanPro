...
export default function AdminMealScans() {
    ...
    useEffect(() => { document.title = 'Admin Meal Scans'; }, []);

    useEffect(() => {
        setLoading(true);
        API.get(`/admin/meal-scans?page=${page}&per_page=20`, { headers: h() })
            .then(r => { setScans(r.data.meal_scans); setTotal(r.data.total); setPages(Math.ceil(r.data.total / r.data.per_page)); })
            .catch(() => {})
            .finally(() => setLoading(false));
    }, [page]);

    const del = (id) => {
        if (!window.confirm('Delete this meal scan?')) return;
        API.delete(`/admin/meal-scans/${id}`, { headers: h() })
    ...
    return (
        <AdminLayout>
            <div style={{ marginBottom: '2rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                ...
                <div>
                    <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#020617', margin: 0 }}>Meal Scans</h1>
                    <p style={{ fontSize: '0.875rem', color: '#64748b', marginTop: '0.125rem' }}>{total} user meal scans</p>
                </div>
            </div>
                    ...
                    <tbody>
                        {scans.map(s => (
                            <tr key={s.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                ...
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b' }}>{s.calories}</td>
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b' }}>{s.protein}g</td>
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b' }}>{s.carbs}g</td>
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b' }}>{s.fat}g</td>
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b', maxWidth: '12rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.notes || '-'}</td>
