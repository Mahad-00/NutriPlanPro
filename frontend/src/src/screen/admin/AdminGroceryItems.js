...
export default function AdminGroceryItems() {
    ...
    useEffect(() => { document.title = 'Admin Grocery Items'; }, []);

    useEffect(() => {
        setLoading(true);
        API.get(`/admin/grocery-items?page=${page}&per_page=20`, { headers: h() })
            .then(r => { setItems(r.data.grocery_items); setTotal(r.data.total); setPages(Math.ceil(r.data.total / r.data.per_page)); })
            .catch(() => {})
    ...
    const del = (id) => {
        ...
        API.delete(`/admin/grocery-items/${id}`, { headers: h() })
    ...
    return (
        <AdminLayout>
            <div style={{ marginBottom: '2rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                ...
                <div>
                    <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#020617', margin: 0 }}>Grocery Items</h1>
                    <p style={{ fontSize: '0.875rem', color: '#64748b', marginTop: '0.125rem' }}>{total} shopping list items</p>
                </div>
            </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
                    <thead>
                        <tr style={{ background: '#f8fafc', textAlign: 'left' }}>
                            {['Name', 'User', 'Email', 'Category', 'Quantity', 'Unit', 'Purchased', ''].map(h => (
                    ...
                    <tbody>
                        {items.map(i => (
                            <tr key={i.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                <td style={{ padding: '0.75rem 1rem', fontWeight: 600, color: '#020617' }}>{i.name}</td>
                                <td style={{ padding: '0.75rem 1rem', fontWeight: 600, color: '#020617' }}>{i.user_name}</td>
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b' }}>{i.email}</td>
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b' }}>{i.category}</td>
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b' }}>{i.quantity}</td>
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b' }}>{i.unit}</td>
                                <td style={{ padding: '0.75rem 1rem' }}>
                                    <span style={{ display: 'inline-block', padding: '0.125rem 0.5rem', borderRadius: '9999px', fontSize: '0.75rem', fontWeight: 600, background: i.purchased ? '#dcfce7' : '#f1f5f9', color: i.purchased ? '#166534' : '#64748b' }}>{i.purchased ? 'Yes' : 'No'}</span>
                                </td>
                                <td style={{ padding: '0.75rem 1rem' }}>
                        ...
                        {!items.length && !loading && (
                            <tr><td colSpan={8} style={{ padding: '3rem', textAlign: 'center', color: '#94a3b8' }}>No grocery items found.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
