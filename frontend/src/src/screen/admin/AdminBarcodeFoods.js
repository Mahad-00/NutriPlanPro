...
export default function AdminBarcodeFoods() {
    ...
    useEffect(() => { document.title = 'Admin Barcode Foods'; }, []);

    useEffect(() => {
        setLoading(true);
        API.get(`/admin/barcode-foods?page=${page}&per_page=20`, { headers: h() })
            .then(r => { setFoods(r.data.barcode_foods); setTotal(r.data.total); setPages(Math.ceil(r.data.total / r.data.per_page)); })
            .catch(() => {})
            .finally(() => setLoading(false));
    }, [page]);

    const del = (id) => {
        if (!window.confirm('Delete this barcode food?')) return;
        API.delete(`/admin/barcode-foods/${id}`, { headers: h() })
    ...
    return (
        <AdminLayout>
            <div style={{ marginBottom: '2rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                ...
                <div>
                    <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#020617', margin: 0 }}>Barcode Foods</h1>
                    <p style={{ fontSize: '0.875rem', color: '#64748b', marginTop: '0.125rem' }}>{total} user-scanned barcode foods</p>
                </div>
            </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
                    <thead>
                        <tr style={{ background: '#f8fafc', textAlign: 'left' }}>
                            {['Name', 'Brand', 'Barcode', 'Serving', 'Calories', 'Protein', 'Carbs', 'Fat', ''].map(h => (
                    ...
                    <tbody>
                        {foods.map(f => (
                            <tr key={f.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                ...
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b' }}>{f.serving_size} {f.serving_unit}</td>
                        ...
                        {!foods.length && !loading && (
                            <tr><td colSpan={9} style={{ padding: '3rem', textAlign: 'center', color: '#94a3b8' }}>No barcode foods found.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
