...
import { Dumbbell, Trash2, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function AdminWorkoutLogs() {
    ...
    useEffect(() => { document.title = 'Admin Workout Logs'; }, []);

    useEffect(() => {
        setLoading(true);
        API.get(`/admin/workout-logs?page=${page}&per_page=20`, { headers: h() })
            .then(r => { setLogs(r.data.workout_logs); setTotal(r.data.total); setPages(Math.ceil(r.data.total / r.data.per_page)); })
            .catch(() => {})
    ...
    const del = (id) => {
        ...
        API.delete(`/admin/workout-logs/${id}`, { headers: h() })
    ...
    return (
        <AdminLayout>
            <div style={{ marginBottom: '2rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                ...
                <div>
                    <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#020617', margin: 0 }}>Workout Logs</h1>
                    <p style={{ fontSize: '0.875rem', color: '#64748b', marginTop: '0.125rem' }}>{total} completed workout logs</p>
                </div>
            </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
                    <thead>
                        <tr style={{ background: '#f8fafc', textAlign: 'left' }}>
                            {['Routine', 'User', 'Email', 'Date', 'Goal', 'Exercises', ''].map(h => (
                    ...
                    <tbody>
                        {logs.map(l => (
                            <tr key={l.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                <td style={{ padding: '0.75rem 1rem', fontWeight: 600, color: '#020617' }}>{l.routine_name}</td>
                                <td style={{ padding: '0.75rem 1rem', fontWeight: 600, color: '#020617' }}>{l.user_name}</td>
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b' }}>{l.email}</td>
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b' }}>{l.date}</td>
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b', textTransform: 'capitalize' }}>{l.goal?.replace('_', ' ')}</td>
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b', maxWidth: '20rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{l.exercises}</td>
                        ...
                        {!logs.length && !loading && (
                            <tr><td colSpan={7} style={{ padding: '3rem', textAlign: 'center', color: '#94a3b8' }}>No workout logs found.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
