...
export default function AdminWeeklyCalendar() {
    ...
    useEffect(() => { document.title = 'Admin Weekly Calendar'; }, []);

    useEffect(() => {
        setLoading(true);
        API.get(`/admin/weekly-calendar?page=${page}&per_page=20`, { headers: h() })
            .then(r => { setEntries(r.data.calendar_entries); setTotal(r.data.total); setPages(Math.ceil(r.data.total / r.data.per_page)); })
            .catch(() => {})
    ...
    const del = (id) => {
        ...
        API.delete(`/admin/weekly-calendar/${id}`, { headers: h() })
    ...
    return (
        <AdminLayout>
            <div style={{ marginBottom: '2rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                ...
                <div>
                    <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#020617', margin: 0 }}>Weekly Calendar</h1>
                    <p style={{ fontSize: '0.875rem', color: '#64748b', marginTop: '0.125rem' }}>{total} weekly calendar entries</p>
                </div>
            </div>
                    ...
                    <tbody>
                        {entries.map(e => (
                            <tr key={e.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                ...
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b', maxWidth: '20rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{e.description || '-'}</td>
