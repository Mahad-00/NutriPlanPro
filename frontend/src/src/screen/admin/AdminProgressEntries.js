...
import { TrendingUp, Trash2, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function AdminProgressEntries() {
    ...
    useEffect(() => { document.title = 'Admin Progress Entries'; }, []);

    useEffect(() => {
        setLoading(true);
        API.get(`/admin/progress-entries?page=${page}&per_page=20`, { headers: h() })
            .then(r => { setEntries(r.data.progress_entries); setTotal(r.data.total); setPages(Math.ceil(r.data.total / r.data.per_page)); })
            .catch(() => {})
    ...
    const del = (id) => {
        ...
        API.delete(`/admin/progress-entries/${id}`, { headers: h() })
    ...
    return (
        <AdminLayout>
            <div style={{ marginBottom: '2rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                ...
                <div>
                    <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#020617', margin: 0 }}>Progress Entries</h1>
                    <p style={{ fontSize: '0.875rem', color: '#64748b', marginTop: '0.125rem' }}>{total} weight / body measurement entries</p>
                </div>
            </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
                    <thead>
                        <tr style={{ background: '#f8fafc', textAlign: 'left' }}>
                            {['User', 'Email', 'Date', 'Type', 'Weight (kg)', 'Waist', 'Chest', 'Hips', 'Arms', 'Thighs', ''].map(h => (
                    ...
                    <tbody>
                        {entries.map(e => (
                            <tr key={e.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                ...
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b' }}>{e.waist ?? '-'}</td>
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b' }}>{e.chest ?? '-'}</td>
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b' }}>{e.hips ?? '-'}</td>
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b' }}>{e.arms ?? '-'}</td>
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b' }}>{e.thighs ?? '-'}</td>
                        ...
                        {!entries.length && !loading && (
                            <tr><td colSpan={11} style={{ padding: '3rem', textAlign: 'center', color: '#94a3b8' }}>No progress entries found.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
