...
export default function AdminDietRecommendations() {
    ...
    useEffect(() => { document.title = 'Admin Diet Recommendations'; }, []);

    useEffect(() => {
        setLoading(true);
        API.get(`/admin/diet-recommendations?page=${page}&per_page=20`, { headers: h() })
            .then(r => { setRecs(r.data.diet_recommendations); setTotal(r.data.total); setPages(Math.ceil(r.data.total / r.data.per_page)); })
            .catch(() => {})
    ...
    const del = (id) => {
        ...
        API.delete(`/admin/diet-recommendations/${id}`, { headers: h() })
            .then(() => setRecs(prev => prev.filter(r => r.id !== id)))
    ...
    return (
        <AdminLayout>
            <div style={{ marginBottom: '2rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                ...
                <div>
                    <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#020617', margin: 0 }}>Diet Recommendations</h1>
                    <p style={{ fontSize: '0.875rem', color: '#64748b', marginTop: '0.125rem' }}>{total} AI diet recommendations</p>
                </div>
            </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
                    <thead>
                        <tr style={{ background: '#f8fafc', textAlign: 'left' }}>
                            {['Name', 'User', 'Email', 'Brand', 'Fit Score', 'Calories', 'Protein', 'Carbs', 'Fiber', 'Meal Type', 'Action', ''].map(h => (
                    ...
                    <tbody>
                        {recs.map(r => (
                            <tr key={r.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                ...
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b' }}>{r.brand || '-'}</td>
                                <td style={{ padding: '0.75rem 1rem' }}><span style={{ background: '#f0fdf4', color: '#166534', fontWeight: 600, padding: '0.125rem 0.5rem', borderRadius: '9999px', fontSize: '0.75rem' }}>{r.fit_score}</span></td>
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b' }}>{r.kcal}</td>
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b' }}>{r.protein}g</td>
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b' }}>{r.carbs}g</td>
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b' }}>{r.fiber}g</td>
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b', textTransform: 'capitalize' }}>{r.meal_type}</td>
                                <td style={{ padding: '0.75rem 1rem', color: '#64748b', textTransform: 'capitalize' }}>{r.action_taken}</td>
                        ...
                        {!recs.length && !loading && (
                            <tr><td colSpan={12} style={{ padding: '3rem', textAlign: 'center', color: '#94a3b8' }}>No diet recommendations found.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
